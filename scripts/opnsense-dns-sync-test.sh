...
: >"$CALLS"
output=$(run_sync cache 10.42.0.2 '{"rows":[{"uuid":"same","enabled":"1","hostname":"cache","domain":"4amlunch.internal","rr":"A","server":"10.42.0.2","description":"Managed by WonkoOS"}]}')
grep -qx 'cache.4amlunch.internal unchanged (10.42.0.2)' <<<"$output"
assert_calls 1

: >"$CALLS"
output=$(run_sync sonarr 10.42.0.3 '{"rows":[]}')
grep -qx 'sonarr.4amlunch.internal added (10.42.0.3)' <<<"$output"
grep -q '/add_host_override$' "$CALLS"
grep -q '/service/reconfigure$' "$CALLS"
assert_calls 3

: >"$CALLS"
output=$(run_sync rutorrent 10.42.0.4 '{"rows":[{"uuid":"change-me","enabled":"1","hostname":"rutorrent","domain":"4amlunch.internal","rr":"A","server":"10.42.0.99","description":"Managed by WonkoOS"}]}')
grep -qx 'rutorrent.4amlunch.internal updated (10.42.0.4)' <<<"$output"
...
: >"$CALLS"
duplicates='{"rows":[
  {"uuid":"one","hostname":"cache","domain":"4amlunch.internal","rr":"A"},
  {"uuid":"two","hostname":"cache","domain":"4amlunch.internal","rr":"A"}
]}'
