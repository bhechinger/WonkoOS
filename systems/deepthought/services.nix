  ...
  services = {
    ...
    udev = {
      extraRules = ''
        ...
        SUBSYSTEM=="input", KERNEL=="event*", ATTRS{id/vendor}=="054c", ATTRS{id/product}=="0ce6", ENV{ID_INPUT_TOUCHPAD}=="1", ENV{LIBINPUT_IGNORE_DEVICE}="1", TAG+="uaccess", SYMLINK+="input/dualsense-touchpad"
      '';
    };
    avahi = {
      enable = true;
